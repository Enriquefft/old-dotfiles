 ...
 *
 * Place trendlog in: $HOME/.local/share/clight/modules.d/ OR, globally, in
 * /usr/share/clight/modules.d/
 */

static bool clightBLCTRL = true; // Set to true if you are using Clight's dbus
                                 // interface to inc/dec backlight level
...
static void receive(const msg_t *msg, const void *userdata) {
  ...
  case AMBIENT_BR_UPD: {
    ...
    if (!skip) {
      ...
      if (clightBLCTRL) {
        sprintf(cmd, "echo '%.3lf %.3lf' >> %s%sdata", up->new, bl_level,
                logpath, mode ? "AC-" : "BAT-");
      } else {
        sprintf(cmd,
                "busctl call org.clightd.clightd "
                "/org/clightd/clightd/Backlight org.clightd.clightd.Backlight "
                "Get s '' | awk '{print \"%.3lf \"$NF}' >> %s%sdata",
                up->new, logpath, mode ? "AC-" : "BAT-");
      }
      system(cmd);
      DEBUG("Ran %s\n", cmd);
    }
